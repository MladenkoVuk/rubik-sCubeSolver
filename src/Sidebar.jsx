...
{ 
 ...
 { 
  ...
  holding: 'Yellow face on TOP. White face on BOTTOM.',
  tasks: [
    'DOT: Use the algo from any side.', 
    'L-SHAPE: Hold at 09:00 (Back and Left).', 
    'LINE: Hold it horizontally (Left to Right).'
  ], 
  algos: [
    { 
      name: 'Cross Algorithm', 
      moves: "F R U R' U' F'", 
      labelMoves: "F R U R' U' F'",
      setup: "F R U R' U' F' U2", 
      instruction: "Front, Right, Up, Right-prime, Up-prime, Front-prime."
    }
  ] 
},
{ 
  ...
  tasks: [
    'STEP 1: If matches are opposite, run R U R\' U R U2 R\'.', 
    'STEP 2: Find two adjacent matches.',
    'STEP 3: Hold one in FRONT and one on the RIGHT.',
    'STEP 4: Run the final algo: U R U R\' U R U2 R\'.'
  ], 
  algos: [
    { 
      name: '1. Get Adjacent Matches', 
      moves: "R U R' U R U U R'", 
      labelMoves: "R U R' U R U2 R'",
      instruction: "Use this when edges are opposite or only one matches."
    },
    { 
      name: '2. Final Alignment', 
      moves: "U R U R' U R U U R'", 
      labelMoves: "U' R U R' U R U2 R'",
      instruction: "Hold matching edges at FRONT and RIGHT, then run this."
    }
  ] 
},
{ 
  id: 'position-corners', 
  stage: '06', 
  title: 'Positioning Corners', 
  color: '#8b5cf6', 
  description: 'Place all corners in their correct "homes". They are in the right spot if their 3 colors match the surrounding centers (e.g., Green-Orange-Yellow corner sits between Green, Orange, and Yellow centers).',
  holding: 'Yellow on TOP. Find one corner that is in the right spot.',
  tasks: [
    'Find a corner that belongs between its three surrounding centers.', 
    'If none are correct: Run the algo once to "create" a correct corner.', 
    'Hold the correct "Anchor" corner in the BOTTOM-RIGHT of the top face.', 
    'Repeat the algo until all 4 corners are in their correct homes even if it is "twisted" (yellow not facing up yet).'
  ], 
  algos: [
    { 
      name: 'Corner Cycle', 
      moves: "U R U' L' U R' U' L", 
      labelMoves: "U R U' L' U R' U' L",
      setup: "L' U R U' L U R' U' ", 
      instruction: "Up, Right, Up-prime, Left-prime, Up, Right-prime, Up-prime, Left."
    }
  ] 
},
{ 
  ...
  color: '#10b981', 
  description: 'The final step! Use R\' D\' R D to rotate the corners. The cube will look scrambled—stay calm and keep going until the end!',
  holding: 'Yellow on TOP. Place an unsolved corner in the BOTTOM-RIGHT.',
  tasks: [
    'Rotate the WHOLE CUBE so an unsolved corner is at the BOTTOM-RIGHT.', 
    'Repeat R\' D\' R D until the Yellow sticker faces UP.', 
    'CRITICAL: Always finish the full 4 moves, including the D!',
    'Rotate ONLY the top layer (U) to bring the next corner to the same spot.',
  ...
  algos: [
    { 
      ...
      moves: "R' D' R D", 
      labelMoves: "R' D' R D",
      instruction: "Right-prime, Down-prime, Right, Down. Repeat until the yellow face is up."
    }
  ] 
}
];
...
const KEY_MAPPINGS = [
  { key: 'R', move: 'R', desc: 'Right' }, { key: 'L', move: 'L', desc: 'Left' }, 
  { key: 'U', move: 'U', desc: 'Up' }, { key: 'D', move: 'D', desc: 'Down' }, 
  { key: 'F', move: 'F', desc: 'Front' }, { key: 'B', move: 'B', desc: 'Back' },
];
  ...
  return (
    <aside className={`sidebar left ${open ? 'visible' : ''}`} style={{ 
      ...
      <div style={{ padding: '24px' }}>
        {/* LOGO SEKCIJA */}
        <h2 style={{ fontSize: '20px', fontWeight: '900', color: '#fff', letterSpacing: '2px', marginBottom: '30px', display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        {/* TIMER & STATS DISPLAY */}
        <div style={{ 
          background: timerRunning ? 'rgba(56, 189, 248, 0.1)' : '#1a1d23', 
          borderRadius: '20px', 
          padding: '24px', 
          textAlign: 'center', 
          marginBottom: '24px',
          border: `1px solid ${timerRunning ? '#38bdf844' : 'transparent'}`,
          transition: 'all 0.3s ease'
        }}>
          <div style={{ fontSize: '42px', fontWeight: '800', color: timerRunning ? '#38bdf8' : '#fff', fontFamily: '"JetBrains Mono", monospace' }}>
            {time}
          </div>
          ...
          <div style={{ marginTop: '20px', paddingTop: '15px', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'center', gap: '20px' }}>
             <div>
                ...
                <div style={{ fontSize: '9px', color: '#555' }}>MOVES</div>
             </div>
             <div>
                <div style={{ fontSize: '18px', fontWeight: '800', color: '#38bdf8' }}>{Math.round(moveCount / (parseFloat(time) || 1) * 10) / 10}</div>
                <div style={{ fontSize: '9px', color: '#555' }}>TPS</div>
             </div>
          </div>
        </div>
        ...
        <button 
          ...
          style={{ 
            ...
            background: timerRunning ? '#f87171' : '#38bdf8', 
            border: 'none', fontWeight: '900', cursor: 'pointer', marginBottom: '12px', color: '#000',
            boxShadow: `0 4px 0 ${timerRunning ? '#b91c1c' : '#0284c7'}`,
      ...
      {/* NOTATION GUIDE (Poboljšana verzija) */}
<div style={{ marginBottom: '35px' }}>
  <div style={{ color: '#444', fontSize: '11px', fontWeight: '900', letterSpacing: '1px', marginBottom: '15px' }}>
    LEARN NOTATION
  </div>
  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
    {[
      { m: 'R', d: 'Right UP', color: '#f87171' },    // Crvena
      { m: "R'", d: 'Right DOWN', color: '#f87171' },
      { m: 'U', d: 'Top LEFT', color: '#facc15' },   // Žuta
      { m: "U'", d: 'Top RIGHT', color: '#facc15' },
      { m: 'F', d: 'Front CLOCK', color: '#4ade80' }, // Zelena
      { m: "F'", d: 'Front COUNTER', color: '#4ade80' },
    ].map((item, i) => (
      <div key={i} style={{ 
        background: '#16191e', 
        padding: '12px 8px', 
        borderRadius: '10px', 
        borderLeft: `3px solid ${item.color}`, // Boja pomaže u prepoznavanju strane
        display: 'flex',
        flexDirection: 'column',
        gap: '2px'
      }}>
  ...
  <div style={{ 
    ...
    <p style={{ color: '#666', fontSize: '10px', margin: 0, lineHeight: '1.5' }}>
      <strong style={{ color: '#38bdf8' }}>TIP:</strong> 
      Imagine looking directly at the face. <strong>"Prime" (')</strong> is always counter-clockwise!
    </p>
  </div>
</div>

        {/* KEYBOARD CONTROLS (KBD STIL) */}
